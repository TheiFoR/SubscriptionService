...
size_t Log::s_currentTrimClassNameLength = 0;
size_t Log::s_currentTrimSubClassNameLength = 0;
size_t Log::s_currentTrimThreadIdLength = 0;

namespace {
constexpr const char* kResetAnsi = "\x1b[0m";

constexpr std::array<const char*, 256> kAnsi256Colors = {
...
const char* colorForThreadId(std::string_view threadId) {
    static std::unordered_map<std::string, std::size_t> s_colorByThreadId;
    static std::size_t s_nextColorIndex = 15;

    const std::string key(threadId);
    auto [it, inserted] = s_colorByThreadId.emplace(key, 0);
    if (inserted) {
        it->second = s_nextColorIndex;
        s_nextColorIndex = (s_nextColorIndex + 53) % kAnsi256Colors.size();
    }

    return kAnsi256Colors[it->second];
}

const char* toLevelString(LogLevel level) {
    switch (level) {
    case LogLevel::Trace: return "\x1b[38;5;233mT\x1b[0m";
    case LogLevel::Debug: return "\x1b[38;5;67mD\x1b[0m";
    case LogLevel::Info: return "\x1b[38;5;152mI\x1b[0m";
    case LogLevel::Warning: return "\x1b[38;5;214mW\x1b[0m";
    case LogLevel::Error: return "\x1b[38;5;1mE\x1b[0m";
    }
    return "I";
}
...
void Log::trace(std::string_view className, std::string_view subClass, std::string_view message) {
    instance().log(LogLevel::Trace, className, subClass, message);
}

void Log::debug(std::string_view className, std::string_view subClass, std::string_view message) {
    instance().log(LogLevel::Debug, className, subClass, message);
}

void Log::info(std::string_view className, std::string_view subClass, std::string_view message) {
    instance().log(LogLevel::Info, className, subClass, message);
}

void Log::warning(std::string_view className, std::string_view subClass, std::string_view message) {
    instance().log(LogLevel::Warning, className, subClass, message);
}

void Log::error(std::string_view className, std::string_view subClass, std::string_view message) {
    instance().log(LogLevel::Error, className, subClass, message);
}

void Log::log(LogLevel level, std::string_view className, std::string_view subClass, std::string_view message) {
    ...
    const std::string threadIdPadded = trimThreadId(threadId);
    const char* threadIdColor = colorForThreadId(threadId);

    line << '[' << makeTimestamp() << ']'
         << " [" << toLevelString(level) << "] "
         << '[' << trimClassName(className) << ']' << '[' << trimSubClassName(subClass) << ']'
         << " [" << threadIdColor << "tid=" << threadIdPadded << kResetAnsi << "] "
         << message
         << '\n';
...
std::string Log::trimClassName(std::string_view className) const
{
    const std::size_t currentLength = className.length();
    const std::size_t targetLength  = std::max(s_currentTrimClassNameLength, currentLength);

    if (currentLength > s_currentTrimClassNameLength) {
        s_currentTrimClassNameLength = currentLength;
    }

    const std::size_t totalPadding = targetLength - currentLength;
    const std::size_t leftPadding  = totalPadding / 2;
    const std::size_t rightPadding = totalPadding - leftPadding;
    ...
    result.append(rightPadding, ' ');

    return result;
}

std::string Log::trimSubClassName(std::string_view subClass) const
{
    const std::size_t currentLength = subClass.length();
    const std::size_t targetLength  = std::max(s_currentTrimSubClassNameLength, currentLength);

    if (currentLength > s_currentTrimSubClassNameLength) {
        s_currentTrimSubClassNameLength = currentLength;
    }
...
std::string Log::trimThreadId(std::string_view threadId) const
{
    const std::size_t currentLength = threadId.length();
    const std::size_t targetLength  = std::max(s_currentTrimThreadIdLength, currentLength);

    if (currentLength > s_currentTrimThreadIdLength) {
        s_currentTrimThreadIdLength = currentLength;
    }
